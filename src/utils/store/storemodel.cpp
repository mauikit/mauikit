...
void StoreModel::setList(StoreList *value)
{
    ...
    if (mList) {
        ...
        connect(this->mList, &StoreList::postItemAppended, this, [=]() { endInsertRows(); });

        connect(this->mList, &StoreList::preItemRemoved, this, [=](int index) { beginRemoveRows(QModelIndex(), index, index); });

        connect(this->mList, &StoreList::postItemRemoved, this, [=]() { endRemoveRows(); });

        connect(this->mList, &StoreList::updateModel, this, [=](int index, QVector<int> roles) { emit this->dataChanged(this->index(index), this->index(index), roles); });

        connect(this->mList, &StoreList::preListChanged, this, [=]() { beginResetModel(); });

        connect(this->mList, &StoreList::postListChanged, this, [=]() { endResetModel(); });
    }
