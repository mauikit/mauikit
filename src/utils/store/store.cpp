...
void Store::contentListResult(Attica::BaseJob *j)
{
    ...
    if (j->metadata().error() == Attica::Metadata::NoError) {
        ...
        foreach (const Attica::Content &p, listJob->itemList()) {
            ...
            list << FMH::MODEL {{FMH::MODEL_KEY::ID, p.id()},
...
void Store::downloadLink(const QString &url, const QString &fileName)
{
    ...
    connect(downloader, &FMH::Downloader::warning, [this](const QString &warning) { emit this->warning(warning); });

    connect(downloader, &FMH::Downloader::fileSaved, [this](const QString &fileName) { emit this->downloadReady(FMH::getFileInfoModel(fileName)); });

    connect(downloader, &FMH::Downloader::done, [=]() { downloader->deleteLater(); });
