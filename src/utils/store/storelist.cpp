...
StoreList::StoreList(QObject *parent)
    : QObject(parent)
{
    ...
    connect(this->store, &Store::warning, [this](const QString warning) { emit this->warning(warning); });

    connect(this->store, &Store::downloadReady, [this](const FMH::MODEL &item) { emit this->downloadReady(FMH::toMap(item)); });
