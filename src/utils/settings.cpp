...
{
    return Settings::local().load(m_key, m_group, m_defaultValue);
}
...
QVariant Settings::load(const QString &key, const QString &group, const QVariant &defaultValue) const
{
    QVariant variant;
...
Settings::Settings(QString app, QString org)
    : QObject(nullptr)
    , m_app(app)
    , m_org(org)
