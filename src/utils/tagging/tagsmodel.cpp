...
void TagsModel::setList(TagsList *value)
{
    ...
    if (mList) {
        ...
        connect(this->mList, &TagsList::postItemAppended, this, [=]() { endInsertRows(); });

        connect(this->mList, &TagsList::preItemRemoved, this, [=](int index) { beginRemoveRows(QModelIndex(), index, index); });

        connect(this->mList, &TagsList::postItemRemoved, this, [=]() { endRemoveRows(); });

        connect(this->mList, &TagsList::updateModel, this, [=](int index, QVector<int> roles) { emit this->dataChanged(this->index(index), this->index(index), roles); });

        connect(this->mList, &TagsList::preListChanged, this, [=]() { beginResetModel(); });

        connect(this->mList, &TagsList::postListChanged, this, [=]() { endResetModel(); });
    }
